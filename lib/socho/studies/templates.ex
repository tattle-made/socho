defmodule Socho.Studies.Templates do
  ...
  def all do
    [consent_gate(), consent_and_instructions(), simple_survey(), image_swipe_task(), iat()]
  end
  ...
  defp consent_gate do
    %{
      ...
      build: fn vars ->
        ...
        consented_fn = "const d = jsPsych.data.get().filter({ tag: \"consent\" }).last(1).values()[0];\nreturn d && d.response === 0;"
        withdrew_fn  = "const d = jsPsych.data.get().filter({ tag: \"consent\" }).last(1).values()[0];\nreturn !d || d.response !== 0;"
